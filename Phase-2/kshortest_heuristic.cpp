...
// Define a large number for distances that are unreachable
constexpr double INFINITY_DIST = std::numeric_limits<double>::infinity();

// --- Constructor ---
KShortestPathsHeuristic::KShortestPathsHeuristic(const Graph& g) : g(g) {}

// --- Helper: Extended Dijkstra's (KSP, allows cycles) to generate candidates ---
// This function finds k_base shortest paths (may contain cycles)
std::vector<HeuristicPath> KShortestPathsHeuristic::generate_candidates(int source, int target, int k_base) {
    ...
    // Priority Queue: holds candidate PathStates, ordered by smallest length
...
// --- Helper: Compute overlap percentage between two paths (by edge set) ---
double KShortestPathsHeuristic::overlap_percent(const HeuristicPath& a,
    ...
    // 1. Identify all edges in path A and store them in a set for quick lookup
    ...
    // 2. Iterate through edges in path B and find shared edges
    for (size_t i = 0; i < b.nodes.size() - 1; ++i) {
        ...
        if (edges_a.count(edge_key)) {
            // Edge (u, v) is shared. Find its length.
            int u_idx = g.node_id_Nodes_index.at(u_id);
    ...
    // 3. Calculate overlap: (Shared Length / Length of Shorter Path) * 100
    double min_length = std::min(a.length, b.length);
    
    // Safety check for division by zero (e.g., if one path is source=target with length 0)
    if (min_length < 1e-6) {
        return 0.0;
    }

    // Overlap is (Length_Intersection / Length_Union) or (Length_Intersection / Length_Min)
    // Based on common literature for diversification, we use Length_Min.
    return (shared_length / min_length) * 100.0;
}

// --- Helper: Score a set of k paths according to a penalty formula ---
// The penalty reflects the quality of the set, balancing length and diversity.
double KShortestPathsHeuristic::total_penalty(const std::vector<HeuristicPath>& paths,
                                              double overlap_threshold) const {
    
    if (paths.empty()) return INFINITY_DIST;
...
// --- Main Function: Compute K diverse paths using a Greedy Strategy ---
std::vector<HeuristicPath> KShortestPathsHeuristic::compute(int source,
                                                           int target,
                                                           int k,
                                                           double overlap_threshold) {
    if (k <= 0) return {};
    if (source == target) return {HeuristicPath{{source}, 0.0}};

    // 1. Generate a large set of candidate paths (k_base is an arbitrary factor, e.g., 5-10 times k)
    ...
    // Step 2a: Always select the single shortest path first (P1)
    result_paths.push_back(candidates[0]);
    // Remove P1 from candidates
    candidates.erase(candidates.begin());
    
    // Step 2b: Iteratively select the best path until K are found
    while (result_paths.size() < (size_t)k && !candidates.empty()) {
        double min_incremental_penalty = INFINITY_DIST;
        ...
        for (size_t i = 0; i < candidates.size(); ++i) {
            ...
            // Note: We could also use the total_penalty function, but this incremental approach is faster.

            // Check if this candidate is the best choice so far
        ...
        // Add the best path to the result set
        if (best_candidate_index != -1) {
            result_paths.push_back(candidates[best_candidate_index]);
            // Remove the selected path from the candidates list
            candidates.erase(candidates.begin() + best_candidate_index);
        } else {
            // Should not happen if candidates is not empty, but break for safety.
            break; 
        }
    }

    // 3. The paths are returned in the order they were selected (shortest/most diverse first).
    return result_paths;
}