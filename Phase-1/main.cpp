        ...
        if (events != queries_json.end() && events->is_array()) {
            for (const auto& event : *events) {
                ...
                if (type_str == "remove_edge") {
                    ...
                    Shortestpath sp_result = (mode_str == "time")? shortestpath_by_time(graph, constraints, source_id, target_id) : shortestpath_by_distance(graph, constraints, source_id, target_id);
                    result["id"] = query_id;
