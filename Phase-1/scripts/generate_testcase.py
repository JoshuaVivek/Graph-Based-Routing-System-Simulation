...
def build_graph_from_overpass(osm_data):
    ...
    # Choose a random number of POIs (e.g., ~10% of nodes get a POI)
    fraction_with_poi = 0.1
    for node in graph_nodes:
        if random.random() < fraction_with_poi:
            node["pois"].append(random.choice(pois_list))
    ...
    def get_or_create_node(osm_id):
        ...
        node_obj = {
            ...
            "lat": n.get("lat", 0.0),
            "lon": n.get("lon", 0.0),
            "pois":[random.choice(pois_list)] if random.random() < 0.1 else [],
        }
...
def make_queries(graph, num_queries=25, seed=42):
    ...
    for _ in range(num_queries):
        ...
        if r < 0.5:
            ...
            while t == s:
                t = rand_node()
            # --- Build forbidden_nodes and forbidden_road_types (can be empty lists) ---
            forbidden_nodes = []
            forbidden_road_types = []
            # 30% chance to have random forbidden_nodes; exclude s and t
            if random.random() < 0.3 and len(node_ids) > 2:
                k_n = random.randint(1, min(2, len(node_ids) - 2))
                possible_nodes = [n for n in node_ids if n not in (s, t)]
                forbidden_nodes = random.sample(possible_nodes, k_n)
            # 30% chance to have forbidden road types
            if road_types and random.random() < 0.3:
                k_rt = random.randint(1, min(2, len(road_types)))
                forbidden_road_types = random.sample(road_types, k_rt)
            # --- Always include both fields, even if empty ---
            constraints = {
                "forbidden_nodes": forbidden_nodes,
                "forbidden_road_types": forbidden_road_types
            }
            events.append({
                "type": "shortest_path",
                "id": qid,
            ...
            metric_choice = random.choice(["euclidean", "shortest_path"])
            events.append({
                "type": "knn",
                "id": qid,
                "poi": random.choice(pois),  # Make sure it's lower-case if your graph uses lower-case!
                "query_point": { "lat": base_node["lat"], "lon": base_node["lon"] },
                "k": random.randint(1, 5),
                "metric": metric_choice  # Or "euclidean" as needed for each query
            })
